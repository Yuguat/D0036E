...
plt.ylabel("$y$", rotation=0, fontsize=18)
plt.legend(loc="upper left", fontsize=14)
plt.title(("Training Degree9 : MSE  "+str(MSE_T)), rotation=0, fontsize=18)
plt.axis([X_min, X_max, 0, 500])
plt.show()
#validation Dataset
X=Val_Data["age"].to_numpy().T
...
plt.ylabel("$Income$", rotation=0, fontsize=18)
plt.legend(loc="upper left", fontsize=14)
plt.title(("Validation Degree9 : MSE  "+str(MSE_val)), rotation=0, fontsize=18)
plt.axis([X_min, X_max, 0, 500])
plt.show()
...
polynomial_regression = Pipeline([("poly_features", PolynomialFeatures(degree=9, include_bias=False)),("lin_reg", LinearRegression()),])
